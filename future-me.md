...
Purpose: Handoff for another Hermes instance to continue fixing squash GUI behavior in emulator for game-movement model.

## User intent
- User wants squash model (`game-movement.elf`) to be testable interactively in emulator GUI.
...
- Squash model artifacts exist in `projects/game-movement/src` including `squash_model.pth`, dataset, eval JSON.
...
Result:
- Return code 0
- Framebuffer dump has exactly 4 active cells (all 255), matching squash top-4 output mapping.
...
1. GUI loop compute chunk is too small (`10000`) for squash model.
2. GUI input mapping is generic; squash model expects `j`/`k`/`space` semantics.
3. GUI does not keep `a0` stable through long compute chunk in same robust way as `--char-code` path.
4. GUI starts from zeroed framebuffer; squash model expects game-like frame state. This causes odd sparse outputs initially.

## Agreed fix strategy
Implement in `emulator_runner.cpp`:

1) Add squash-aware GUI mode automatically for `game-movement` ELF.
2) Add `--gui-cycles <N>` CLI option.
3) Use large default GUI chunk for squash if not specified (e.g. `9000000`).
4) Keep current key code stable across compute chunk by repeatedly writing `a0` each step.
5) In squash GUI mode:
   - Map keys:
     - Up arrow / W -> `'j'`
     - Down arrow / S -> `'k'`
     - Space -> `' '`
   - Ignore unrelated keys.
6) Seed initial framebuffer before first render in squash mode:
   - ball at `(x=0, y=10)`
...
3. Validate with these tests:
   ...
   - Optional tune:
     - `--gui-cycles 8872000`
4. If possible, add/adjust black-box test coverage for GUI squash mode key mapping + visible frame update.
