"""
Export movement_model.pth to emulator JSON intermediate format.
"""
...
from movement_model import PlayerMovementNetwork
...
def build_intermediate(model: PlayerMovementNetwork) -> dict[str, Any]:
    return {
        "metadata": {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export movement model to emulator JSON format")
    parser.add_argument("--checkpoint", default="movement_model.pth", help="Input checkpoint path")
    parser.add_argument(
        "--output",
        default="movement_generator.json",
