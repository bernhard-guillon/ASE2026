...
// Raw ASCII keycode preserved in callee-saved register s1 (like char_gen)
register uint32_t key_code asm("s1");
...
static inline void run_forward_pass(void) {
    const uint8_t *model_bytes = (const uint8_t *)movement_model_data;
    const uint32_t *model_u32 = (const uint32_t *)movement_model_data;
    ...
    uint32_t layer_table_offset = MOVEMENT_HEADER_SIZE;
    uint32_t weights_base = MOVEMENT_HEADER_SIZE + num_layers * MOVEMENT_LAYER_ENTRY_SIZE;
    uint32_t biases_base = weights_base + total_weights * 4u;

    for (uint32_t layer_idx = 0; layer_idx < num_layers; layer_idx++) {
        const uint32_t *layer = (const uint32_t *)(model_bytes + layer_table_offset + layer_idx * MOVEMENT_LAYER_ENTRY_SIZE);
...
void inference_loop(void) {
    ...
    for (;;) {
        // Read a0 (set by emulator for each keypress)
        uint32_t input_val;
        __asm__ volatile ("mv %0, a0" : "=r"(input_val));

        *done_flag = 0;

        // Build input: state one-hot + action one-hot
        for (uint32_t i = 0; i < MOVEMENT_INPUT_SIZE; i++) input[i] = 0.0f;
        input[current_state] = 1.0f;

        uint32_t action_id = 4;
        if (input_val == 'h') action_id = 2;
        else if (input_val == 'j') action_id = 1;
        else if (input_val == 'k') action_id = 0;
        else if (input_val == 'l') action_id = 3;
        input[BOARD_CELLS + action_id] = 1.0f;
        ...
        for (uint32_t i = 1; i < MOVEMENT_OUTPUT_SIZE; i++) {
            if (output[i] > max_val) {
                max_val = output[i];
                max_idx = i;
            }
        ...
        if (max_idx < BOARD_CELLS) {
            current_state = max_idx;
        }

        // Update framebuffer
        for (uint32_t i = 0; i < BOARD_CELLS; i++) fb[i] = 0;
        fb[current_state] = 255;
