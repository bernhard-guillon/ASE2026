#!/usr/bin/env python3
"""
Verilator smoke test for mega combined model (squash + counter-chargen + router).
Mirrors test_mega_combined.py but runs on verilator_runner.
"""
...
def _build_targets() -> None:
    ...
    build = subprocess.run(
        ["cmake", "--build", str(BUILD_DIR), "--target", "verilator_runner", "mega_combined_elf",
    ...
    assert build.returncode == 0, (
        f"Failed to build verilator+mega targets.\n"
        f"stdout:\n{build.stdout}\n"
        f"stderr:\n{build.stderr}"
    )
...
def test_verilator_mega_elf_exists() -> None:
...
def test_verilator_mega_produces_framebuffer() -> None:
...
def test_verilator_mega_is_deterministic() -> None:
...
def test_verilator_mega_chargen_mode() -> None:
...
def test_verilator_mega_framebuffer_size() -> None:
