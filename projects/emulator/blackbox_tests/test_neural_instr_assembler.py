...
def test_nmatvec8xp2_encoding():
    word = _assemble_and_extract_word(ASM_ROOT / "nmatvec8xp2" / "test.s")
    assert word == _expected_word(7, 6, 5, 0, 0, 0x7B)


def test_nmatvec8xp3_encoding():
    word = _assemble_and_extract_word(ASM_ROOT / "nmatvec8xp3" / "test.s")
    assert word == _expected_word(8, 6, 5, 0, 0, 0x7B)


def test_nmatvec8xp4_encoding():
    word = _assemble_and_extract_word(ASM_ROOT / "nmatvec8xp4" / "test.s")
    assert word == _expected_word(9, 6, 5, 0, 0, 0x7B)
