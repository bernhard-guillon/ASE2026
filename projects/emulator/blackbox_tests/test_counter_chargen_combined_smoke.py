#!/usr/bin/env python3
"""
Smoke test for block-diagonal combined counter+chargen model.
Verifies the merged MLP produces framebuffer output and the counter advances.
"""
...
def _build_targets() -> None:
    build = subprocess.run(
        ["cmake", "--build", str(BUILD_DIR), "--target", "emulator_runner", "counter_chargen_combined_elf", "--", "-j4"],
        cwd=EMULATOR_DIR,
        capture_output=True,
        text=True,
        timeout=300,
    ...
    assert build.returncode == 0, (
        f"Failed to build combined targets.\n"
        f"stdout:\n{build.stdout}\n"
        f"stderr:\n{build.stderr}"
    )
...
def test_combined_counter_advances() -> None:
    ...
    # Counter is mod 255, so it may wrap; the delta should equal
    # the number of iterations completed in 4M cycles (mod 255)
    delta = (v10 - v6) % 255
    assert delta != 0, f"Counter wrapped to same value: {v6} -> {v10}"


def test_combined_is_deterministic() -> None:
...
def test_combined_produces_framebuffer_output() -> None:
...
def test_combined_framebuffer_is_deterministic() -> None:
...
def test_combined_different_cycles_different_framebuffer() -> None:
