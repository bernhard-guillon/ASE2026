...
class ModelCompiler:
    ...
    def generate_assembly(self, json_path: str, output_asm: str, temp_bin: str = None, 
                         with_bootloader: bool = True) -> str:
        """
        Generate RISC-V assembly with embedded binary data.
        
        The assembly file includes:
        1. .data section with .incbin directive pointing to binary data
        2. Section headers and alignment directives
        3. Bootloader code for memory initialization (Phase 2) or skeleton (Phase 1)
        
        Args:
            json_path: Path to JSON intermediate file
            output_asm: Path to output .s assembly file
            temp_bin: Optional path to temporary .bin file for .incbin directive
                     If None, uses output_asm with .bin extension
            with_bootloader: If True, generate full bootloader code (Phase 2).
        ...
        # Load and compile JSON
        self.load_json_intermediate(json_path)
        self.generate_binary_format()
        
        # Determine binary file path
        if temp_bin is None:
            # Use same path as output_asm but with .bin extension
        ...
        num_layers = len(self.layers)
        binary_size = len(self.binary_data)
        
        asm_content = self._generate_asm_header(model_type, num_layers, binary_size)
        asm_content += self._generate_data_section(temp_bin, binary_size)
        asm_content += self._generate_text_section(include_bootloader=with_bootloader)
